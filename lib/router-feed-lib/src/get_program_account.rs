            ...
            .send::<OptionalContext<Vec<RpcKeyedAccount>>>(
                ...
                json!([
                    program_id.to_string(),
                    {
                        ...
                        "dataSlice": config.account_config.data_slice,
                        "minContextSlot": config.account_config.min_context_slot,
                    }
                ]),
            )
            .await;

        // Handle uncompressed program accounts
        match result {
            Ok(OptionalContext::Context(response)) => {
                info!("Received uncompressed data for {}", program_id);
                ...
                for key_account in response.value {
                    ...
                    let ui_account: UiAccount = key_account.account.try_into().unwrap();
                    let account: Account = Account {
                        lamports: ui_account.lamports,
                        data: match ui_account.data {
                            UiAccountData::Binary(data, encoding) => {
                                match encoding {
                                    UiAccountEncoding::Base58 => solana_sdk::bs58::decode(data).into_vec().unwrap(),
                                    UiAccountEncoding::Base64 => base64::prelude::BASE64_STANDARD.decode(data).unwrap(),
                                    _ => panic!("Unsupported encoding: {:?}", encoding),
                                }
                            },
                            UiAccountData::LegacyBinary(data) => bs58::decode(data).into_vec().unwrap(),
                            _ => panic!("Unexpected UiAccountData format"),
                        },
                ...
                info!(
                    "Processed snapshot for {} with {} accounts",
                    program_id,
                    snap_result.len()
                );
            }
