...
impl HttpServer {
    ...
    >(
        ...
        let other_amount_threshold = if swap_mode == SwapMode::ExactOut {
            (route.in_amount as f64 * (10_000f64 + input.slippage_bps as f64 * 1.1) / 10_000f64).ceil()
                as u64
        } else {
            ((route.out_amount as f64 * (10_000f64 - input.slippage_bps as f64 * 0.9)) / 10_000f64)
                .floor() as u64
        };
    ...
    async fn list_mints_handler<
        TRouteProvider: RouteProvider + Send + Sync + 'static,
    >(
        route_provider: Arc<TRouteProvider>,
    ) -> Result<Json<Value>, AppError> {
        // Assuming your RouteProvider has a method `get_all_mints`
        // that returns a list of Pubkey

        let mints = route_provider.get_all_mints()?;
        let mints_with_decimals: Vec<serde_json::Value> = mints
            .into_iter()
            .map(|(pubkey, decimals)| serde_json::json!({
                "address": pubkey.to_string(),
                "decimals": decimals
            }))
            .collect();
        let json_response = serde_json::json!({ "mints": mints_with_decimals });
        Ok(Json(json_response))
    }
    ...
    >(
        ...
        router = router.route(
            "/liquidity",
            routing::get(move |form| Self::liquidity_handler(lp.clone(), form)),
        );
        ...
        router = router.route("/mints", routing::get({
            let rp = rp.clone();
            move || async move { Self::list_mints_handler(rp.clone()).await }
        }));
...
fn reprice<TAccountProvider: AccountProvider + Send + Sync + 'static>(
    ...
    for step in &route.steps {
        ...
        match step.edge.quote(&prepared_quote, &account_provider, amount) {
            Ok(quote) => {
                amount = quote.out_amount;
            },
            Err(e) => {
                return Err(anyhow::anyhow!("Failed to reprice route"));
            }
        }
    }
    Ok(amount)
