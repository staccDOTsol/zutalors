...
fn start_chaindata_updating(
    ...
    tokio_spawn("chain_data", async move {
        ...
        let blocking_rpc = BlockingRpcClient::new("http://god.ligmanode.com:8899".to_string());
        ...
        loop {
            tokio::select! {
                ...
                res = slot_updates.recv() => {
                    ...
                    // Update Clock Sysvar
                    if let Ok(clock_account) = blocking_rpc.get_account(&clock_pubkey) {
                        writer.update_account(
                            clock_pubkey,
                            mango_feeds_connector::chain_data::AccountData {
                                slot: slot_update.slot,
                                write_version: 0, // This might need adjustment
                                account: clock_account.into(),
                            },
                        );
                    }
                }
